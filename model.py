...
def print_parameters(model, title):
    """
    Function to print the total number of parameters and the number of non-zero parameters in the model.

    Parameters
    ----------
    model : nn.Module
        The model whose parameters need to be printed.
    title : str
        The title to be printed before printing the parameters.
    """
    print(title)
    total_params = 0
    non_zero_params = 0
    for name, param in model.named_parameters():
        if 'weight' in name:
            ...
            print(f"{name}: Total Parameters: {param.numel()}, Non-zero Parameters: {(param.data != 0).sum().item()}")
    print(f"Total Parameters: {total_params}, Non-zero Parameters: {non_zero_params}\n")


def prune_model(model, pruning_rate):
    """
    Function to prune the model using magnitude-based weight pruning.
    ...
    pruning_rate : float
        The pruning rate to apply (between 0 and 1).
    """
    for layer in model.modules():
        if isinstance(layer, nn.Linear):
            prune.l1_unstructured(layer, name='weight', amount=pruning_rate)

    # Remove the pruned weights' history so that the original model can be used later
    for layer in model.modules():
        if isinstance(layer, nn.Linear):
...
average_val_NMI_score = []
average_test_NMI_score = []
...
class KPGNN():
    ...
    def infer(train_i, i, data_split, metrics, embedding_save_path, loss_fn, train_indices=None, model=None,
              loss_fn_dgi=None, indices_to_remove=[], pruning_allowed = False):

        global average_val_NMI_score, average_test_NMI_score
        ...
        # sim_label = torch.IntTensor(data.similar_labels)
        ...
        if args.remove_obsolete == 1:
            ...
            if ((args.resume_path is not None) and (not args.resume_current) and (i == args.resume_point + 1) and (
                    i > args.window_size)) \
                    or (
                    indices_to_remove == [] and i > args.window_size):  # Resume indices_to_remove from the last maintain block
        ...
        if (args.resume_path is not None) and (not args.resume_current) and (
            ...
            if args.use_dgi:
                ...
                if pruning_allowed:
                    ...
                    print_parameters(model, "Parameters before pruning:")

                    # Set the pruning rate
                    pruning_rate = 0.5  # Modify this value as desired

                    # Prune the model
                    prune_model(model, pruning_rate)

                    # Print parameters after pruning
                    print_parameters(model, "Parameters after pruning:")
        ...
        max_test_nmi = np.max(all_test_nmi)
        average_test_NMI_score.append(max_test_nmi)

        return model

    # Train on initial/maintenance graphs, t == 0 or t % window_size == 0 in this paper
    def initial_maintain(train_i, i, data_split, metrics, embedding_save_path, loss_fn, model=None, loss_fn_dgi=None, pruning_allowed=False):
        args = args_define.args
        # make dir for graph i
        ...
        max_val_nmi = np.max(all_vali_nmi)
        average_val_NMI_score.append(max_val_nmi)
